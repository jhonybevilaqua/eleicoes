@echo off
REM Faz o GC-PRODUCAO.bat subir sozinho (minimizado) quando alguem logar.
cd /d "%~dp0"
set "PASTA=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup"
set "ATALHO=%PASTA%\gctse.lnk"
choice /c SN /m "Criar atalho de inicializacao para GC-PRODUCAO.bat"
if errorlevel 2 exit /b 0
powershell -NoProfile -Command ^
  "$s=(New-Object -ComObject WScript.Shell).CreateShortcut('%ATALHO%');" ^
  "$s.TargetPath='%~dp0GC-PRODUCAO.bat'; $s.WorkingDirectory='%~dp0';" ^
